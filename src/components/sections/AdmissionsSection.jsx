...
function StepCard({ step, iconName, title, description, align = 'left' }) {
  ...
  return (
    <div className="group rounded-2xl p-6 border max-w-xs w-full transition-all duration-300 hover:-translate-y-1"
      ...
      <h3 className="font-semibold text-base mb-2 group-hover:text-[#F5C842] transition-colors" style={{ color:'var(--text-primary)' }}>{title}</h3>
  ...
  return (
    <section id="admissions" className="py-12 md:py-24 px-4 sm:px-6 md:px-16" style={{ backgroundColor:'var(--bg-section)' }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div ref={timelineRef} className="hidden lg:block relative max-w-4xl mx-auto">
          ...
          {ADMISSION_STEPS.map((step, i) => {
            ...
            return (
              <div key={step.step} data-step className="grid grid-cols-[1fr_60px_1fr] mb-12 items-center gsap-hidden">
                ...
                <div className="w-12 h-12 rounded-full bg-gold-gradient flex items-center justify-center font-bold text-black text-sm mx-auto z-10" style={{ boxShadow:'var(--shadow-gold-sm)' }}>
                  {step.step}
                </div>
          ...
          {ADMISSION_STEPS.map((step, i) => {
            ...
            return (
              <div key={step.step} className="relative mb-8 last:mb-0">
                <div className="absolute -left-8 top-5 w-9 h-9 rounded-full bg-gold-gradient flex items-center justify-center font-bold text-black text-xs" style={{ boxShadow:'var(--shadow-gold-sm)' }}>{step.step}</div>
                <div className="group rounded-2xl p-5 border transition-all duration-300"
                  style={{ backgroundColor:'var(--bg-card)', borderColor:'var(--border-subtle)' }}
                  onMouseEnter={e => e.currentTarget.style.borderColor='var(--border-gold)'}
                  onMouseLeave={e => e.currentTarget.style.borderColor='var(--border-subtle)'}>
                  <Icon size={20} className="text-[#D4A017] mb-2" />
        ...
        <div className="text-center mt-14">
          <p className="text-sm mb-5" style={{ color:'var(--text-muted)' }}>Ready to take the first step? Applications are reviewed within 24 hours.</p>
          <a href="/admissions" className="inline-flex items-center gap-2 bg-gold-gradient text-black font-bold px-10 py-4 rounded-full text-base transition-transform duration-200 hover:scale-105 hover:-translate-y-0.5 active:scale-95" style={{ boxShadow:'var(--shadow-gold)' }}>
            <GraduationCap size={18} />Start Your Application
          </a>
        </div>
      </div>
    </section>
