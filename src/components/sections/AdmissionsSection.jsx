  ...
  return (
    <div className="group rounded-2xl p-5 md:p-6 border w-full transition-all duration-300 hover:-translate-y-1"
      ...
      <div className={`flex mb-3 ${align === 'right' ? 'justify-end' : 'justify-start'}`}>
        <div className="w-9 h-9 md:w-10 md:h-10 rounded-xl flex items-center justify-center" style={{ background:'rgba(212,160,23,0.12)', border:'1px solid rgba(212,160,23,0.20)' }}>
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ADMISSION_STEPS.forEach((_, i) => {
        ...
        if (!step) return;
        const isLeft = i % 2 === 0;
        gsap.fromTo(step, {x: isLeft ? -50 : 50, opacity: 0},
  ...
  return (
    <section id="admissions" className="py-10 md:py-16 px-4 sm:px-6 md:px-16" style={{ backgroundColor:'var(--bg-section)' }}>
      <div className="max-w-7xl mx-auto">
        ...
        {/* Alternating Left/Right with Center Line - SAME FOR BOTH */}
          ...
          {ADMISSION_STEPS.map((step, i) => {
            ...
            return (
              <div key={step.step} className="relative mb-6 last:mb-0">
                {/* Circle on the line */}
                <div className="absolute -left-7 top-4 w-8 h-8 rounded-full bg-gold-gradient flex items-center justify-center font-bold text-black text-xs z-10" style={{ boxShadow:'var(--shadow-gold-sm)' }}>
                  {step.step}
                </div>
