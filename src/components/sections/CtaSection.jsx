  ...
  return (
    <section ref={sectionRef} className="relative py-14 md:py-28 px-4 sm:px-6 md:px-16 overflow-hidden">
      ...
      <div className="relative max-w-4xl mx-auto text-center z-10">
        ...
        <h2 data-cta-el className="font-display text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black text-white leading-tight mb-3 sm:mb-5 gsap-hidden">
          Your Tech Career Starts<span className="block text-gold-gradient">Right Here, Right Now</span>
        </h2>
        <p data-cta-el className="text-white/70 text-sm sm:text-base md:text-lg leading-relaxed max-w-2xl mx-auto mb-6 sm:mb-10 gsap-hidden">
        ...
        <div data-cta-el className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center gsap-hidden">
          <a href="/admissions" className="inline-flex items-center justify-center gap-2.5 bg-gold-gradient text-black font-black px-6 sm:px-10 py-3 sm:py-4 rounded-full text-sm sm:text-base transition-transform duration-200 hover:scale-105 hover:-translate-y-1 active:scale-95 w-full sm:w-auto"
            style={{ boxShadow: '0 20px 50px rgba(212,160,23,0.45)' }}>
            <GraduationCap size={18} />Apply Now — Free Consultation
          </a>
          <a href="tel:+922111524786" className="inline-flex items-center justify-center gap-2.5 border-2 border-white/30 text-white hover:border-[#D4A017]/60 hover:text-[#F5C842] font-semibold px-6 sm:px-10 py-3 sm:py-4 rounded-full text-sm sm:text-base transition-all duration-300 w-full sm:w-auto">
            <Phone size={16} />+92-21-111-524-786
          </a>
        </div>
        <div data-cta-el className="flex flex-wrap justify-center gap-3 sm:gap-x-6 gap-y-2 mt-8 sm:mt-12 gsap-hidden">
          {TRUST.map((b) => (
            <span key={b} className="flex items-center gap-1.5 text-white/45 text-[11px] sm:text-sm">
              <CheckCircle size={12} className="text-[#D4A017]/60" />{b}
            </span>
          ))}
        </div>
      </div>
